...
Однако, мне гораздо удобнее писать текст поста у себя на компьютере. Как оказалось, редактор VSCode, который мы используем в колледже, имеет встроенную функцию предпросмотра для файлов на языке Markdown. Пишу посты теперь в этом редакторе. Готовый файл с постом можно отправить на GitHub через веб-интерфейс, но я предпочитаю использовать систему контроля версий [git](https://git-scm.com). В клонированном (скопированном) репозитории у меня на компьютере создание коммита с новым постом и его отправка на GitHub занимают полминуты или меньше. Как только GitHub получает новый коммит, он автоматически запускает сборку и публикацию проекта на хостинге GitHub Pages.
