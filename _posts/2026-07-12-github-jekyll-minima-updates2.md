...
Я выбрал тему оформления [minima](https://github.com/jekyll/minima). Она позиционируется как базовая, простая, минималистичная (это отражено и в названии), универсальная. Поскольку в разработке я предпочитаю простоту и минималистичность, то мне такой настрой авторов темы понравился. Позже я узнал, что это одна из официальных тем Jekyll (создана его авторами) и является темой по умолчанию (устанавливается по команде `jekyll new`, если вы работаете с этим инструментом на своем компьютере из командной строки; это я пока не делал, так как работаю по-другому).
